...
impl InMemoryStore {
pub fn new(cap: usize) -> Self { Self { inner: RwLock::new(BTreeMap::new()), wal: None, cache: RwLock::new(CLruCache::new(cap.try_into().unwrap())) } }
...
Ok(Self { inner: RwLock::new(m), wal: Some(RwLock::new(wal)), cache: RwLock::new(CLruCache::new(cap.try_into().unwrap())) })
}
}
...
self.inner.write().insert((space.clone(), key), val); Ok(())
}
fn del(&self, space: &Space, key: &[u8]) -> Result<()> { self.cache.write().pop(&(space.clone(), key.to_vec())); self.inner.write().remove(&(space.clone(), key.to_vec())); Ok(()) }
